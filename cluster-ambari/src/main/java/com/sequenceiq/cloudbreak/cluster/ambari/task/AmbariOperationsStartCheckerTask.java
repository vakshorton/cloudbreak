...
import com.sequenceiq.cloudbreak.cluster.ClusterBasedStatusCheckerTask;
...
public class AmbariOperationsStartCheckerTask extends ClusterBasedStatusCheckerTask<AmbariOperations> {
    ...
    public boolean checkStatus(AmbariOperations t) {
        ...
        for (Entry<String, Integer> request : installRequests.entrySet()) {
            ...
            BigDecimal installProgress = Optional.ofNullable(ambariClient.getRequestProgress(request.getValue())).orElse(AmbariOperationsStatusCheckerTask.PENDING);
            LOGGER.info("Ambari operation start: '{}', Progress: {}", request.getKey(), installProgress);
            if (AmbariOperationsStatusCheckerTask.FAILED.compareTo(installProgress) == 0) {
                boolean failed = true;
                for (int i = 0; i < MAX_RETRY; i++) {
                    if (ambariClient.getRequestProgress(request.getValue()).compareTo(AmbariOperationsStatusCheckerTask.FAILED) != 0) {
                        failed = false;
                        break;
                    }
                }
            ...
            if (AmbariOperationsStatusCheckerTask.PENDING.compareTo(installProgress) == 0) {
                return false;
            }
        }
        return true;
